...
2. maybe impossible! (e.g. if the code of the function arguments is
   not available.  That is, if we want to make a library of
   higher-order functions.)

*** Transformation: Defunctionalisation (explicit <<closures>>)
file:Template/ExplicitClosures.hs
file:Final/ExplicitClosures.hs
**** Example
- Source
#+begin_src haskell
map :: (a -> b) -> List a -> List b
...
** Continuations
source code: file:Final/Continuations.hs
...
explicitly passing continuations to every function. Each function will
take an extra argument (the continuation). Instead of returning a
result directly, the result is passed to the continuation.

The first step in this transformation is to decide an order of
execution. In this case we will evaluate the arguments first.

The transformation can then be summarized as follows, where 'eff' is
...
After this transformation, there remains ONLY tail calls!  Hence,
doing the transformation is similar to converting code to use explicit
gotos.

Remark: in essence, we have explained gotos only in terms of
'functional' features; that is, in terms of the substitution model.
...
:            k (n * fn)  -- Mult. closure
:
: test = fact id 12 -- the test function is not transformed.
...
*** TODO Application: accumulator side effect
** TODO State
Using continuation we can implement get and put
...
Note: saving/restoring the process state is done by the functional
language (closures).
*** Exercises
