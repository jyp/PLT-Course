...
instance Monad IP where
  (>>=) = andThen
  return x = x -- when x does not depend on the state:
#+end_src
...
And say we insert 13 in it. The new tree /shares/ most of its contents
with the old one.
...
** Exercise: implement semaphore using channels & processes
See the [[http://en.wikipedia.org/wiki/Semaphore_(programming)][wikipedia article]].

server: manage the semaphore.
queries: - get(channel to wake)
         - put
state of the server?
