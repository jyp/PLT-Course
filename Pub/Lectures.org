...
** Transformation: inlining procedure calls (forward reference: the substitution model)
...
Solution: every variable must be copied to a temporary before proceeding.
...
We can understand recursion in the von Neumann model, by using a stack.
