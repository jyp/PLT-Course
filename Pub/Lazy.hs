...
-- NOTE: the function does not compute its body until the argument is applied:
...
-- xs is evaluated to a value; but it is a function, and so its body is not evaluated (yet).
hd (Cons x xs) = x
