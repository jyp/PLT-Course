...
\author[Sibylle Schupp] % (optional, use only with lots of authors)
...
\pgfdeclareimage[height=0.5cm]{university-logo}{logo-chalmers-L}
...
\textsf{http://www.it-c.dk/courses/PFOO/F2003/6.pdf}
\end{itemize}
\end{frame}
...
\begin{frame}[fragile]
\frametitle{History}
\framesubtitle{http://www.oreilly.com/pub/a/oreilly/news/languageposter_0504.html}
...
\item CPU takes instruction from memory.
      ...
      back to memory. 
\item Instructions result in changes in the state of the machine.
The chances are represented in memory.
\end{itemize}
\begin{tabular}{ll}
...
\framesubtitle{http://www.engin.umd.umich.edu/CIS/course.des/cis400/cobol/seccob.html}
...
\framesubtitle{http://www.engin.umd.umich.edu/CIS/course.des/cis400/algol/average.html}
\begin{cplus3}
...
{\small 
\begin{tikzpicture}
\draw[blue] (0,0)  rectangle (1.2,0.5);
%\pgftext[at={\pgfpoint{0.65cm}{0.3cm}}]{} ;
\pgftext[at={\pgfpoint{2.5cm}{0.3cm}}]{de-allocated} ;


\draw (0,1.0)  rectangle (1.2,1.5);
\pgftext[at={\pgfpoint{0.5cm}{1.25cm}}]{n:1} ;

%\draw[red] (0,1.5)  rectangle (1.2,2.0);
%\pgftext[at={\pgfpoint{0.5cm}{1.75cm}}]{n:  } ;

\draw (0,2.0)  rectangle (1.2,2.5);
\pgftext[at={\pgfpoint{0.5cm}{2.25cm}}]{n:0 } ;

\draw (0,3.0)  rectangle (1.2,3.5);
\pgftext[at={\pgfpoint{0.5cm}{3.2cm}}]{n: ? } ;

\draw[blue] (0,4.0)  rectangle (1.2,4.5);
\pgftext[at={\pgfpoint{2.5cm}{4.3cm}}]{not allocated} ;
\end{tikzpicture}
}
\end{tabular}
\end{tabular}
...
\framesubtitle{}
Any von Neumann language  (implicitly) distinguishes between
the address of a variable and the value stored there. 
\begin{itemize}
\item The address is also called the \textit{l-value} of a variable,
the content its \textit{r-value} (due to its position
in an assignment).
\begin{cplus3}
     x := x + 1;
\end{cplus3}
\item Most languages do not make the distinction explicit.
%
\mycomment{
\begin{itemize}
\item Exception: ML
...
\item Most languages do not permit explicit access to the l-value
\begin{itemize}
...
Assume the assignment $    x : = y; $


\begin{itemize}
\item Copy semantics:  the fields of $y$ are copied to the
corresponding fields of $x$.
\item Reference semantics:  the reference is copied; $x$ and $y$
become \textit{aliases} of another. 
\end{itemize}
...
\draw (0,0.5)  rectangle (1.2,1.0);
\pgftext[at={\pgfpoint{0.5cm}{0.75cm}}]{c: 3 } ;


\draw (0,0.0)  rectangle (1.2,0.5);
\pgftext[at={\pgfpoint{0.5cm}{0.25cm}}]{d: 4} ;



% right one 
\draw (3,0)  rectangle (4.2,0.5);
\pgftext[at={\pgfpoint{3.5cm}{0.25cm}}]{a: 10} ;
\draw[->] (2.9, 0.25) -- (0.8, 0.25);

\draw (3,0.5)  rectangle (4.2,1.0);
\pgftext[at={\pgfpoint{3.5cm}{0.75cm}}]{b: 11} ;
\draw[->] (3.0, 0.75) -- (0.8, 0.75);
...
\pgftext[at={\pgfpoint{3.5cm}{1.75cm}}]{d: 4 } ;

\draw (3,2.0)  rectangle (4.2,2.5);
\pgftext[at={\pgfpoint{3.5cm}{2.25cm}}]{c: 3 } ;
\end{tikzpicture}
}
\end{tabular}
\end{tabular}
...
{\small 
\begin{tikzpicture}
\draw (0,0)  rectangle (1.7,0.5);
\pgftext[at={\pgfpoint{0.65cm}{0.3cm}}]{AVG} ;

\draw (0,0.5)  rectangle (1.7,1.0);
\pgftext[at={\pgfpoint{0.65cm}{0.75cm}}]{SUM} ;



\draw (0,1.0)  rectangle (1.7,1.5);
\pgftext[at={\pgfpoint{0.65cm}{1.25cm}}]{I } ;

\draw (0,1.5)  rectangle (1.7,2.0);
\pgftext[at={\pgfpoint{0.65cm}{1.75cm}}]{ret} ;


\draw (0,2.0)  rectangle (1.7,2.5);
\pgftext[at={\pgfpoint{0.8cm}{2.25cm}}]{addr(ARR)} ;

\draw (0,2.5)  rectangle (1.7,3.0);
\pgftext[at={\pgfpoint{0.8cm}{2.75cm}}]{addr(N)} ;
...
%\item In that context, ARs are sometimes called \textit{stack frames}.
\end{itemize}
...
\end{itemize}
%In \cpp, heap-based variables must be deallocated by hand. 
...
\frametitle{Pointers}
\framesubtitle{}
For a pointer type, the r-value of an instance is 
either an l-value or a special
value (e.g., null). 
\bigskip
