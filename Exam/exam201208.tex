...
You are facing the task to translate an application written in Erlang
into an imperative language like C. List a sequence of transformations
seen in the course that, if applied in the order given, will transform
the Erlang program to an imperative program easy to write in C syntax.
...
\item Did you perform tail-call optimisation in your translation?
\item Is it at all possible to do so? Qualify your answer if necessary.
\end{itemize}
...
\item Do not change (optimise) the algorithm.
\end{itemize}
...
a given person.  That is, given a person \texttt{c}, construct the
...
Hint: begin by writing the types of the functions \texttt{ancestors}
and \texttt{parents}.

Write your answer in a functional language.
