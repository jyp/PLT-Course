...
The following interpreter for a simple functional language is missing
...
\item Consider the code to write in place of \texttt{(1)}, which is
  ...
  \texttt{e}. Under which condition can you assume that the recursive
  evaluation of \texttt{f} is going to yield the expected constructor?
   \points{2}

\item Write the for \texttt{(1)}. Hint: you may assume the condition
  stated at the previous point. \points{3}

\item Consider the code to write in place of \texttt{(2)}, which
  evaluates a lambda abstraction to a a closure. What constructor
  should you return? \points{1}

\item What is the representation of a closure that you use in this
  case? \points{2}
...
\item Fun
\item a Haskell closure (we depend on the underlying host language
  mechanism to evaluate functions).
...
Here is an implementation of it, which uses an explicit stack:
...
\item What are the line(s) that must be simply deleted?  \points{2}
\item One more line may be left in the code, but may also be
  deleted. Which one? \points{2}
\item Which line has to be replaced? \points{2}
\item What is the code replacing the above line? \points{4}
