...
\question{*,@1} Give a meaningful type to the following values.
...
\question{*,@4} define an algebraic type for arithmetic expressions (+, *, ...)

\begin{ans}
  \inputminted[firstline=3,lastline=7]{haskell}{fp/Ex51and52.hs}
\end{ans}

\question{*,@4} Define a simple interpreter for the above type; that
...
\question{*,@4} Translate the above 2 structures to an OO language. (Hint: One
...
\question{*,@4} Translate the interpreter to an OO
...
\question{*,@4} Translate the interpreter to an OO language. You must use
'instanceof'.
...
\question{*,@4} Define a function \texttt{f} following this spec: given a
...
\question{*,@4} Assuming a function max : \texttt{(Int × Int) → Int}, define the function \texttt{f}.

\begin{ans}
  \inputminted[firstline=15,lastline=16]{haskell}{fp/Ex56to59.hs}
\end{ans}

\question{*,@4} Define a function \texttt{max'}, by currifying the function \texttt{max}.
