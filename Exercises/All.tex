...
\newenvironment{ans}{\paragraph{Answer~to~exercise~\arabic{question}}}{}
\newcommand{\removed}[1]{}
...
\lstnewenvironment{haskell}
    {\lstset{language=haskell,frame=tb}}
    {}
...
\question{**,@1} Explain the meaning of the following types. (Hint: what
...
\begin{ans}
  \begin{enumerate} 
   ...
   \item a function that evals a function in some environment
   \item same as 3
...
\begin{ans}
  See Dan Ros\'{e}n's implementation.
\end{ans}
...
\begin{verbatim}
    ...
    /* note the absence of &'s in the signature */
    void swap(int a, int b) {
        int t = a;
        a = b;
        b = a;
    ...
    /* will work because a new value is created and returned */
...
\question{*\correctedLastYear 3} define an algebraic type for arithmetic expressions
\question{*\correctedLastYear 3} define a simple interpreter for the above type

\question{*\correctedLastYear 3} Translate the above 2 structures to an OO language. (Hint: One
class corresponds to leaves, one to branching.)

\question{*\correctedLastYear 3} Translate the interpreter. You are not allowed to use 'instanceof'

\question{*} Translate the interpreter. You must use 'instanceof'.

\subsection{Currification and partial application}

\question{*\correctedLastYear 3} Define a function $\mathsf{f}$ following this spec: given a
integer, return it unchanged if it is greater than zero, and zero
otherwise. (The type must be Int → Int.)

\question{*\correctedLastYear 3} Assuming a function max : (Int × Int) → Int, define the function $\mathsf{f}$.
...
\question{*\correctedLastYear 3} Unfold the following expressions:
\begin{enumerate}
\item map f
\item filter ($>=$ 0)
\item foldr [] (++)
