...
\question{*,@3} Define a simple interpreter for the above type; that
...
\question{*} Translate the interpreter to an OO language. You must use
'instanceof'.
...
\question{*} Define a function \texttt{f} following this spec: given a
...
\question{*} Assuming a function max : \texttt{(Int × Int) → Int}, define the function \texttt{f}.

\begin{ans}
  \inputminted[firstline=15,lastline=16]{haskell}{fp/Ex56to59.hs}
\end{ans}

\question{*} Define a function \texttt{max'}, by currifying the function \texttt{max}.
...
as an imperative algorithm.
Use \verb!State! from the \emph{ContinueState} lecture.
% You should use \emph{IP} from the lecture notes.
...
\question{*\correctedLastYear 4} Rewrite the Haskell version, in such a way that
the continuations are made visible, i.e. eliminate your
usage of \verb!State!.

\subsection{Laziness}
There are no lazy languages that permit mutation.  \question{*** \correctedLastYear 5,@4} Why not?
...
\question{**,@4} Repeat the above steps with the algorithm to compute the
...
\begin{ex}{*\correctedLastYear5,@4} Translate \textsf{sieve} to use explicit thunks.
  \begin{ans}
