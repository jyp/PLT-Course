...
\begin{enumerate}
    ...
    \item pop both returns a value and modifies its implicit parameter.
...
You can consider that \textsf{error} is a member of all sorts. 
\end{question}

\begin{question}{0}
Change \textsf{pop} to return a pair of result and updated stack. Adapt the axioms.
\end{question}
...
axioms:
   ...
   search(push(s,o),o) = 1
   search(push(s,e),o) = if o == e then 1 else 1 + search(s,o)
\end{verbatim}
\end{longanswer}
\end{question}
...
\item What is a \textit{monoid}, mathematically? 
...
\answer{
(int,+,0), (int,*,1), (float, +,0), (list, append, emptyList), (string, concat, emptyString)
\ldots}
...
\item What if the method \textsf{op} \emph{would} compile? 
(Imagine for the purpose of the next question that the compiler did not raise the error
message about \textsf{op}.)
