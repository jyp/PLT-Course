...
Consider that \textsf{error} is a member of all sorts, which you can use to
...
for \textsf{Circle} and \textsf{Shape}.
...
Just leave these algorithms out, and focus on how the correct implementation
is selected.
...
\item One has to create a different (visitor) class for things that ``should'' be methods.
...
What are the benefits of the \textsf{increment2} solution?
\answer{
...
\noindent Consider the following block expression:
\begin{java}
|anArray sum|
sum := 0
anArray := #(4 55 19 23).
