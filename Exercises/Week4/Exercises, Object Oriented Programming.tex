...
Decide how you want to handle Java exceptions. \jp{We should decide for students here. I have no idea what this means}

\begin{question}{0}
Adjust your algebraic specification from 1. so that \textsf{java.util.Stack}
can be considered an implementation. Change, and extend, the signature from part 1 so that it ``fits'' the Java stack. Summarize and briefly justify all changes.
\end{question}


\begin{question}{0}
Provide axioms for the \textsf{search} operation. Revise the
axioms for \textsf{empty, push, peek} as necessary; briefly justify.
\end{question}


\begin{question}{0}
Read the API for the \textsf{pop} method and explain the
problem for the corresponding operation in the algebraic specification (1-2
sentences).
\end{question}

\begin{question}{Optional 0} 
provide axioms for \textsf{pop} that reflect the
...
\begin{question}{0}
Complete the algebraic specification. 
\end{question}
...
\item Briefly recap: what is a \textit{monoid}, mathematically? 
...
Note that the monoid class we defined does not match the mathematical
...
\item The C\#-code above does not compile. 

\begin{question}{0} 
Explain the error message in terms of co-/contra-/nonvariance.
\end{question}


\item What if the method \textsf{op} \textit{would} compile? (Imagine for the
purpose of the next question that the compiler is run with the type checker
disabled.)

\begin{question}{0}
Introduce values \textsf{a,b} of appropriate types so that
\textsf{a.op(b)}, if run would result in a run-time error.
\end{question}

\item What if the method \textsf{id} would compile? 

\begin{question}{0}
Can you introduce a value \textsf{a} of appropriate type so that
\textsf{a.id()}, if run, would result in a run-time error?
...
\item The corresponding program in Java behaves differently, in conformance
with the Java Language Specification (JLS).

\begin{question}{0}
Briefly explain how Java deals with the problem, and back your answer
...
\jp{This is extremely fuzzy, I wouldn't know what to answer}
